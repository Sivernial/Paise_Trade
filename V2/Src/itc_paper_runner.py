...
class ITCRefinedSession:
    def __init__(self):
        ...
        logger.info(f"Initializing Phase 42: ITC REFINED MTFA SESSION")
        
        # Init Strategy with Refined Parameters
        self.strategy = ITCRefinedStrategy(params={
            'leverage': 4.0,
            'profit_target': 0.015, # 1.5% for ITC
            'stop_loss': 0.005      # 0.5% for ITC
        })
    ...
    def run(self):
        ...
        logger.info(f"ITC REFINED MTFA ONLINE: Monitoring {SYMBOL}")
        stream.start()
