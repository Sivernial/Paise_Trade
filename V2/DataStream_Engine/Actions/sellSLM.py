...
class SellSLM(BaseAction):
    ...
    def execute(self, symbol: str, quantity: int, trigger_price: float,
        ...
        try:
            ...
            logger.info(f"Sell SL-M order placed: {order_id} for {quantity} {symbol} @ Trigger: {trigger_price}")
            return order_id
            
        except Exception as e:
            logger.error(f"Error placing Sell SL-M order: {e}")
            raise
