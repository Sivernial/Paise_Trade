...
def run_backtest(target_date: datetime, days: int = 1):
    ...
    strategy = SilverSentinel3TFStrategy(params=strategy_params)
    ...
    # Fetch 10m, 30m, and 1h data with warmup
    ...
    df_30m_full = fetcher.fetch_historical_data("SILVERBEES", test_start_date - timedelta(days=20), test_end_date + timedelta(days=1), interval="30minute")
    ...
    if df_30m_full.index.tz: df_30m_full.index = df_30m_full.index.tz_localize(None)
    ...
    for i in range(len(df_exec)):
        ...
        # Forest: 30m up to current time
        full_df_30m = df_30m_full[df_30m_full.index <= current_time]
        # Sky: All 1h data up to the last hour candle BEFORE current_time
        ...
        # Strategy input format for 3TF: {'SILVERBEES': {'10minute': df_10m, '30minute': df_30m, '1hour': df_1h}}
        strategy_data = {
            "SILVERBEES": {
    ...
    print("\n" + "="*50)
    print(f"3TF MTFA BACKTEST: SILVERBEES")
