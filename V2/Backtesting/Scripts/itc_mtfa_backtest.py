...
try:
    ...
    from Algorithms.itc_refined_strategy import ITCRefinedStrategy
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("itc_backtest.log"),
        logging.StreamHandler(sys.stdout)
    ]
)
logger = logging.getLogger("ITC_MTFA_Backtest")
...
def run_backtest(target_date: datetime, days: int = 1):
    ...
    strategy_params = {'symbol': 'ITC', 'forest_ema_period': 20, 'tree_ema_period': 9, 'leverage': 4.0}
    strategy = ITCRefinedStrategy(params=strategy_params)
    ...
    logger.info(f"Fetching data for ITC from {test_start_date.strftime('%Y-%m-%d')} to {test_end_date.strftime('%Y-%m-%d')}...")
    df_10m_full = fetcher.fetch_historical_data("ITC", test_start_date - timedelta(days=10), test_end_date + timedelta(days=1), interval="10minute")
    df_1h_full = fetcher.fetch_historical_data("ITC", test_start_date - timedelta(days=40), test_end_date + timedelta(days=1), interval="60minute")
    ...
    for i in range(len(df_exec)):
        ...
        strategy_data = {"ITC": {"10minute": full_df_10m, "1hour": full_df_1h}}
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='ITC MTFA Backtest')
