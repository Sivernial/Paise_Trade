...
class IndigoRefinedStrategy(BaseStrategy):
    """
    Refined Multi-Timeframe Analysis (MTFA) Strategy for INDIGO.
    ...
    def __init__(self, params: dict = None):
        ...
        self.symbol = self.params.get('symbol', 'INDIGO')
        ...
        # Safety Features (INDIGO is more volatile than ITC)
        self.profit_target_pct = self.params.get('profit_target', 0.025) # 2.5% for INDIGO
        self.stop_loss_pct = self.params.get('stop_loss', 0.01)         # 1.0% for INDIGO
    ...
    def generate_signals(self, data: Dict[str, Dict[str, pd.DataFrame]], 
        ...
        # Phase 49 Fix: Use current LTP from Trees timeframe for Forest Bias
        # to ensure gap-awareness.
        ...
        logger.info(f"STRATEGY MONITOR | {self.symbol} | Price: {price:.2f} | EMA9: {curr_ema_tree:.2f} | VWAP: {curr_vwap:.2f} | Forest: {forest_bias}")
